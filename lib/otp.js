...
// أرقام ثابتة لمراجعي المتاجر (آبل وجوجل) — تتجاوز Twilio وتقبل كود ثابت.
// كل رقم مربوط بكوده الخاص فقط (مو أي كود يشتغل لأي رقم مراجعة)، وما ينفع
// يشتغل بدون ضبط الاثنين بمتغيرات البيئة عشان ما نفتح باب خلفي بالغلط.
// للسجلات: آخر ٤ أرقام تكفي لتطابق الشكوى مع صاحبتها، بلا ما نكتب أرقام
// جوالات المستخدمات كاملة بسجلات المزوّد.
...
function reviewAccounts() {
  return [
    { phone: normalizeKuwaitPhone(process.env.APPLE_REVIEW_PHONE || ""), code: process.env.APPLE_REVIEW_CODE },
    { phone: normalizeKuwaitPhone(process.env.GOOGLE_REVIEW_PHONE || ""), code: process.env.GOOGLE_REVIEW_CODE },
  ].filter((a) => a.phone && a.code);
}

export function isReviewPhone(e164) {
  return reviewAccounts().some((a) => a.phone === e164);
}

export function isReviewCode(e164, code) {
  return reviewAccounts().some((a) => a.phone === e164 && String(code).trim() === a.code);
}
