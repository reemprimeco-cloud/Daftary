...
export function installAuthFetch() {
  ...
  window.fetch = (input, init = {}) => {
    ...
    // التطبيق يعرّف عن نفسه بكل طلب، عشان السيرفر يقدر يرفض الدفع بالبطاقة
    // جواه — بيع محتوى رقمي داخل التطبيق بغير مشتريات المتجر يعرّضه للشطب،
    // وهذي قاعدة آبل وGoogle Play معاً. نرسل المنصة الفعلية مو "ios" ثابتة.
    if (window.Capacitor?.isNativePlatform?.()) {
      headers.set("x-daftary-platform", window.Capacitor.getPlatform?.() || "native");
    }
