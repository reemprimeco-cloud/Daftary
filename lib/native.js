...
// motherId معرّف UUID، وهو نفس الصيغة اللي تطلبها آبل لـappAccountToken —
// فنربط معاملة آبل بحساب ولي الأمر عندنا، وهذا يخلي الاستعادة تجيب مشترياته
// هو بالذات بدل أي شي على الجهاز.
...
// onlyCurrentEntitlements تقصر النتيجة على اشتراكات حساب آبل الحالي النشطة،
...
export async function nativeRestorePurchases(accountToken) {
  ...
  return (list || []).map((t) => t.transactionId).filter(Boolean);
}
