...
// ————— الشراء داخل التطبيق —————
// الشراء يمر عبر StoreKit بإضافة Capacitor، ونرجّع معرّف المعاملة فقط —
// السيرفر هو اللي يتحقق منه عند آبل قبل ما يمنح الرصيد. الثقة بما يرسله
// العميل وحده تعني إن أي أحد يقدر يمنح نفسه اشتراكاً.
//
// لو الإضافة مو مثبّتة بعد بمشروع Xcode، نرمي رسالة واضحة بدل ما نفشل بصمت.
export async function nativePurchase(productId) {
  const store = plugin("InAppPurchase") || plugin("Purchases");
  if (!store) {
    throw new Error("الشراء غير متاح بهذه النسخة من التطبيق — حدّثه من آب ستور.");
  }
  const result = await store.purchase({ productId });
  return result?.transactionId || result?.transaction?.id || null;
}

// استعادة المشتريات — آبل تطلب زراً لها بأي تطبيق فيه اشتراكات (بند 3.1.1).
export async function nativeRestorePurchases() {
  const store = plugin("InAppPurchase") || plugin("Purchases");
  if (!store) return [];
  const result = await store.restorePurchases();
  return result?.transactions || result?.results || [];
}
