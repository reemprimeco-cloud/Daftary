...
export async function finalizeCharge(chargeId, expectedMotherId = null) {
  ...
  // scope يخلي صفحة العودة تعرف أي اشتراك اشتُري — رسالة «رصيد الأسئلة
  // مقسّم بين أبنائك» تخص المعلم الذكي وحده، وعرضها بعد شراء اشتراك
  // دفتري الشامل يربك المشترية عن اللي دفعت مقابله فعلاً.
  if (isAppAccess) {
    ...
    return { ...granted, scope: "app", label: product.label };
  }
  ...
  return { ...granted, scope: "teacher", label: product.label };
}
