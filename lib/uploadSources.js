...
// صور المعلم الذكي عمرها أقصر — قرار صاحبة التطبيق (٢١ سبتمبر): ٢٤ ساعة
// تكفي لتعرف وش أرسلت بنفس الجلسة، وما تخلّي صور الكتب تتكدّس بالتخزين.
export const AI_IMAGE_TTL_HOURS = 24;
...
export async function storeSourceImages(sb, { motherId, childId, images, ttlHours }) {
  try {
    ...
    const hours = ttlHours || SOURCE_TTL_DAYS * 24;
...
// روابط عرض لمجموعة مصادر دفعة وحدة (محادثة المعلم الذكي). المنتهية
// تُستثنى هنا بلا انتظار الكرون: الكرون يحذف الملفات مرة باليوم، لكن
// «٢٤ ساعة» لازم تكون ٢٤ ساعة بعين الأم لا ٤٨.
export async function signedUrlsBySource(sb, ids) {
  ...
  const pairs = await Promise.all(
    rows.map(async (row) => {
      ...
      if (!path) return null;
      const { data } = await sb.storage.from(BUCKET).createSignedUrl(path, SIGNED_URL_SECONDS);
      return data?.signedUrl ? [row.id, data.signedUrl] : null;
    })
  );
