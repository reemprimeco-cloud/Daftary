...
// عكس consumeQuestion لما يفشل نداء النموذج بعد الخصم: السؤال انخصم من
// رصيد الأم بلا ما توصلها إجابة، والعطل عندنا مو عندها. نرجّعه للمصدر
// نفسه اللي خُصم منه (اشتراك / رصيد / تجربة).
export async function refundQuestion(childId, source) {
  const sb = supabaseAdmin();
  const { data: e } = await sb.from("entitlements").select("used_questions, credit_questions, trial_used").eq("child_id", childId).maybeSingle();
  if (!e) return;
  const patch =
    source === "subscription" ? { used_questions: Math.max(0, (e.used_questions || 0) - 1) }
    : source === "credits" ? { credit_questions: (e.credit_questions || 0) + 1 }
    : source === "trial" ? { trial_used: Math.max(0, (e.trial_used || 0) - 1) }
    : null;
  if (patch) await sb.from("entitlements").update({ ...patch, updated_at: new Date().toISOString() }).eq("child_id", childId);
}
