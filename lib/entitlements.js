...
//
// الرصيد مقفول على مستوى العائلة وينقسم على الأبناء، فتكلفتنا مسقوفة به
// مهما كان عددهم — ولذلك maxStudents إرشاد للواجهة فقط، مو شرط نمنع فيه.
export const PRODUCTS = {
  "app.reemora.daftary.teacher.family3": {
    kind: "subscription", questions: 600, maxStudents: 3, priceKwd: 6, label: "١–٣ طلاب",
  },
  "app.reemora.daftary.teacher.family7": {
    kind: "subscription", questions: 1000, maxStudents: 7, priceKwd: 10, label: "٤–٧ طلاب",
  },
...
// الباقات المعروضة بشاشة الاشتراك، مرتبة، مع معرّف كل منتج.
...
// الباقة الموصى بها حسب عدد أبناء ولي الأمر — لو عددهم يتجاوز أكبر باقة
// نرشّح الأكبر (تشتغل عادي، بس الرصيد ينقسم على عدد أكثر).
export function recommendedTier(childrenCount) {
  return SUBSCRIPTION_TIERS.find((t) => childrenCount <= t.maxStudents) || SUBSCRIPTION_TIERS.at(-1);
}
