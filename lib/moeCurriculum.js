...
// خادم الوزارة خارج سيطرتنا: بلا مهلة، لو تأخر أو علّق يبقى سؤال الأم
// معلّقاً معه لين تنتهي مهلة الدالة كلها. ست ثوانٍ كافية، وبعدها نجاوب بلا
// مواد الوزارة (المستدعون يلتقطون الخطأ ويكملون).
const MOE_TIMEOUT_MS = 6000;

export async function fetchMoeSubjects(gradeId) {
  const res = await fetch(`${MOE_BASE}/api/LibraryLookups/EducationSubjects/${gradeId}`, { signal: AbortSignal.timeout(MOE_TIMEOUT_MS) });
  if (!res.ok) return [];
...
export async function searchMoeLibrary({ stageId, gradeId, subjectId, term }) {
  const res = await fetch(`${MOE_BASE}/api/librarysearch`, {
    method: "POST",
    signal: AbortSignal.timeout(MOE_TIMEOUT_MS),
