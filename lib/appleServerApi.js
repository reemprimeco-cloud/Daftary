...
// نطلب من آبل ترسل إشعاراً تجريبياً لعنوان الإشعارات المسجّل عندها —
// الطريقة الوحيدة للتأكد إن الربط صحيح قبل أول تجديد أو استرجاع حقيقي.
// نجرّب الإنتاج ثم الاختبار: مفتاح واحد يخدم البيئتين، والفرق أي عنوان
// مسجّل تُنادى.
export async function requestTestNotification() {
  ...
  for (const env of ["production", "sandbox"]) {
    let res;
    try {
      res = await fetch(`${APPLE_API[env]}/inApps/v1/notifications/test`, {
        method: "POST",
        headers: { Authorization: `Bearer ${jwt}` },
      });
    } catch (e) {
    ...
    if (res.ok) {
      ...
      return { token: body.testNotificationToken, environment: env };
    }
    if (env === "sandbox") {
      return { error: `آبل ردّت ${res.status}: ${(await res.text()).slice(0, 200)}` };
    }
  }
  return { error: "تعذّر طلب الإشعار التجريبي" };
}

// آبل تسجّل كل محاولة تسليم وردّ خادمنا عليها — هذا اللي يثبت إن الإشعار
// وصلنا فعلاً وقبلناه، مو بس إننا طلبناه.
export async function getTestNotificationStatus(token) {
  const jwt = await appleJwt();
  if (!jwt) return { error: "مفاتيح آبل غير مُعدّة بالسيرفر" };
  ...
  for (const env of ["production", "sandbox"]) {
    let res;
    try {
      res = await fetch(`${APPLE_API[env]}/inApps/v1/notifications/test/${token}`, {
        headers: { Authorization: `Bearer ${jwt}` },
      });
    } catch (e) {
      return { error: `تعذّر الاتصال بآبل: ${e.message}` };
    ...
    if (res.ok) {
      ...
      return {
        ...
        attempts: history.map((a) => ({
          ...
          result: a.sendAttemptResult,
        })),
        delivered: history.some((a) => a.sendAttemptResult === "SUCCESS"),
      };
    }
    if (env === "sandbox") return { error: `آبل ردّت ${res.status}` };
  }
  return { error: "تعذّرت قراءة حالة الإشعار" };
}
