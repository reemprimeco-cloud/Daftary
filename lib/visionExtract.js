...
async function detectRotation(image, { motherId, childId }) {
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            {
              type: "text",
              text: `لا تقرئي محتوى الصورة. انظري فقط لاتجاه الكتابة فيها: هل السطور أفقية والحروف معتدلة كما تُقرأ عادةً؟ أرجعي JSON فقط بلا أي كلمة أخرى: {"rotation": N} حيث N هي درجة الدوران مع عقارب الساعة اللازمة لجعل الكتابة معتدلة — 0 إذا كانت معتدلة أصلاً، وإلا 90 أو 180 أو 270.`,
            },
            imageBlock(image),
          ],
        }],
...
export async function extractFromImages({ images, prompt, motherId, childId, feature }) {
  ...
  const content = [{ type: "text", text: GUARD_RULE + prompt }, ...images.map(imageBlock)];
