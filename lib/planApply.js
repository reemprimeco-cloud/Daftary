...
export async function applyPlanItems(sb, childId, items, sourceId = null) {
  ...
  for (const e of items.tasks || []) {
    ...
    if (existing) {
      ...
      if (inserted) known.set(key, { id: inserted.id, edited_by_user: !!e.editedByUser });
      addedTasks++;
    }
  }
  ...
  for (const r of items.requirements || []) {
    ...
    if (existing) {
      if (!existing.edited_by_user) {
        await sb.from("requirements").update({ due_date: dueDate, source_text: text(r.sourceText) }).eq("id", existing.id);
        updatedReqs++;
      }
    } else {
