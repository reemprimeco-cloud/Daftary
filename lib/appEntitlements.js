...
// أسبوعان سماح للجميع، لكن من نقطة بداية مختلفة حسب توقيت التسجيل — عشان
// ولي أمر يسجّل يوم قبل الإلزام ما يفاجئه الاشتراك فوراً، وولي أمر قديم
// ياخذ نفس الأسبوعين من تاريخ التفعيل نفسه:
//   - مسجَّل قبل APP_PAYWALL_ENFORCE_AT  → إلزامه من نفس هذا التاريخ
//   - مسجَّل بعده (مستخدم جديد بعد الإطلاق) → إلزامه بعد أسبوعين من تسجيله هو
// نأخذ الأبعد زمنياً من (تاريخ التفعيل) و(تسجيله + ١٤ يوم) — فالقديم يرجع
// تاريخ التفعيل (لأن تسجيله+١٤ أقدم منه)، والجديد يرجع تسجيله+١٤.
...
export async function hasAppAccess(motherId) {
  // لو الميزة مطفّاة أصلاً أو ما تحدّد تاريخ إلزام بعد، ما فيه أي استعلام —
  // نفس ضمان "بلا كلفة أداء وقت الإطفاء" السابق.
  if (!appPaywallEnabled() || !globalEnforceAt()) return { allowed: true, phase: "off", enforceAt: null };

  const sb = supabaseAdmin();
  const { data: mother } = await sb.from("mothers").select("created_at").eq("id", motherId).maybeSingle();
  const { phase, enforceAt: at } = appPaywallState(mother?.created_at);
