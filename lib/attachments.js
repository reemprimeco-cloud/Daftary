...
export async function storeAttachment(sb, { motherId, childId, kind, images, note }) {
  ...
  // **الاستبدال** (قرار صاحبة التطبيق ٢٦ سبتمبر): رفعة جديدة لنفس
  // الطالب/ة ونفس النوع تحل محل السابقة بدل ما تتراكم — الخطة الأسبوعية
  // وحدة بكل أسبوع، وجدول الحصص واحد. وبنفس قاعدة رفع جدول الحصص: **ما
  // نحذف القديم إلا بعد نجاح حفظ الجديد**، فرفعة فاشلة ما تخسّرها الاثنين.
  // وفشل الحذف ما يفشل الرفعة — الجديد محفوظ، وأسوأ ما يصير صف زايد.
  let replaced = 0;
  try {
    const { data: old } = await sb
      ...
      .neq("id", id);
    if (old?.length) {
      const oldPaths = old.flatMap((r) => r.paths || []);
      if (oldPaths.length) await sb.storage.from(BUCKET).remove(oldPaths);
      await sb.from("plan_attachments").delete().in("id", old.map((r) => r.id));
      replaced = old.length;
    }
  } catch (e) {
    console.warn("storeAttachment: replacing previous failed:", e.message);
  }
