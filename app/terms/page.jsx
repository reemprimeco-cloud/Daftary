  ...
  return (
    <div dir="rtl" className="app-scroll" style={{ background: "#FAF7F2", padding: "20px 20px 60px" }}>
      ...
      <main style={{ maxWidth: 680, margin: "0 auto", background: "white", borderRadius: 20, padding: "32px 26px", boxShadow: "0 1px 3px rgba(0,0,0,.06)", lineHeight: 1.9, color: "#374151", fontSize: 15 }}>
        ...
        <Section title="اشتراك دفتري">
          <p style={p}>
            استخدام دفتري — الجداول والواجبات والمتطلبات والحفظ والدرجات والتذكيرات — يتطلّب اشتراكاً سنوياً حسب
            عدد أبنائك المسجَّلين. المعلم الذكي له اشتراكه المستقل الموضَّح تحت، ولا يدخل ضمن هذا الاشتراك.
          </p>
          <ul style={ul}>
            {APP_TIERS.map((t) => (
              <li key={t.productId} style={li}>
                <strong>{t.label}:</strong> {t.priceKwd} د.ك للسنة.
              </li>
            ))}
          </ul>
          <p style={p}>
            الاشتراك يتجدد تلقائياً سنوياً، ويشمل كل أبنائك المسجَّلين ضمن الباقة المختارة. إذا زاد عدد أبنائك عن
          ...
          <p style={{ ...p, background: "#FDF3E7", padding: "12px 14px", borderRadius: 12, color: "#8C6027", fontSize: 14 }}>
            <strong>فترة سماح {APP_PLAN.GRACE_DAYS === 14 ? "أسبوعين" : `${APP_PLAN.GRACE_DAYS} يوماً`}:</strong> إذا
            كان حسابك مسجَّلاً قبل تفعيل هذا الاشتراك، يبقى استخدامك مجانياً لمدة {APP_PLAN.GRACE_DAYS === 14 ? "أسبوعين" : `${APP_PLAN.GRACE_DAYS} يوماً`} من
            تاريخ التفعيل، مع تنبيه داخل التطبيق بالتاريخ الذي يبدأ منه الاشتراك المطلوب. وإذا سجَّلتِ حساباً جديداً
            بعد التفعيل، تحصلين على نفس فترة السماح من تاريخ تسجيلك أنتِ. بعد انتهاء فترتك، يتطلّب الاستمرار في
            استخدام دفتري اشتراكاً سارياً.
          </p>
        </Section>
        ...
        <Section title="الدفع والتجديد والإلغاء">
          <p style={p}>
            تتم جميع المدفوعات — لاشتراك دفتري ولاشتراك المعلم الذكي — عبر <strong>مشتريات آبل داخل التطبيق</strong>.
            نحن لا نستقبل ولا نخزّن ولا نعالج أي بيانات بطاقة بنكية إطلاقاً.
          </p>
          <ul style={ul}>
